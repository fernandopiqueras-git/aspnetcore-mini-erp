...
public class SuppliersController(AppDbContext db) : Controller
{
 ...
 [HttpPost,ActionName("Delete"),ValidateAntiForgeryToken] public IActionResult DeleteConfirmed(int id){var x=db.Suppliers.Find(id);if(x is null)return NotFound();if(db.PurchaseOrders.Any(o=>o.SupplierId==id))return RedirectToAction(nameof(Details),new{id});db.Remove(x);db.SaveChanges();return RedirectToAction(nameof(Index));}
