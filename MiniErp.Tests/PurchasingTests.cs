...
public class PurchasingTests
{
 ...
 [Fact] public void Receive_IncreasesStockOnlyOnce(){using var db=Database();Seed(db);var o=new PurchaseOrder{Number="PC-1",SupplierId=1,Status=PurchaseOrderStatus.Confirmed,Lines=[new(){ProductId=1,Quantity=5,UnitPrice=2}]};db.Add(o);db.SaveChanges();var c=new PurchaseOrdersController(db);c.Receive(o.Id);c.Receive(o.Id);Assert.Equal(15m,db.Products.Single().Stock);Assert.Equal(PurchaseOrderStatus.Received,o.Status);}
 ...
 [Fact] public void Supplier_DeleteIsBlockedWhenUsed(){using var db=Database();Seed(db);db.Add(new PurchaseOrder{Number="PC-1",SupplierId=1});db.SaveChanges();var result=new SuppliersController(db).DeleteConfirmed(1);Assert.IsType<RedirectToActionResult>(result);Assert.Single(db.Suppliers);}
 ...
 private static void Seed(AppDbContext db){db.Add(new Supplier{Id=1,Name="Proveedor",TaxId="B45000007"});db.Add(new Product{Id=1,Sku="A",Name="Artículo",Stock=10,IsActive=true});db.SaveChanges();}
}