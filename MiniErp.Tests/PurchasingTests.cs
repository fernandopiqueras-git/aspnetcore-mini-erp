...
public class PurchasingTests
{
 ...
 [Fact] public void Supplier_CreateRejectsFormattedDuplicateTaxId(){using var db=Database();db.Add(new Supplier{Name="Existente",TaxId="B45000007"});db.SaveChanges();var controller=new SuppliersController(db);var result=controller.Create(new Supplier{Name="Nuevo",TaxId="B-45.000.007"});Assert.IsType<ViewResult>(result);Assert.False(controller.ModelState.IsValid);Assert.Single(db.Suppliers);}
 ...
 [Theory][InlineData(0,2,0)][InlineData(-1,2,0)][InlineData(1,-1,0)][InlineData(1,2,-1)][InlineData(1,2,101)] public void Create_RejectsInvalidLineValuesWithoutPersisting(decimal quantity,decimal unitPrice,decimal discount){using var db=Database();Seed(db);var model=new PurchaseOrderFormViewModel{Number="PC-1",SupplierId=1,WarehouseId=1,Lines=[new(){ProductId=1,Quantity=quantity,UnitPrice=unitPrice,DiscountPercentage=discount}]};var controller=new PurchaseOrdersController(db);var result=controller.Create(model);Assert.IsType<ViewResult>(result);Assert.False(controller.ModelState.IsValid);Assert.Empty(db.PurchaseOrders);}
 [Fact] public void Create_RejectsEmptyLinesWithoutPersisting(){using var db=Database();Seed(db);var model=new PurchaseOrderFormViewModel{Number="PC-1",SupplierId=1,WarehouseId=1,Lines=[]};var controller=new PurchaseOrdersController(db);var result=controller.Create(model);Assert.IsType<ViewResult>(result);Assert.False(controller.ModelState.IsValid);Assert.Empty(db.PurchaseOrders);}
